...
from old.datatypes import Vector, Particle, Pos
from copy import copy

def tdperlin(size, octaves, seeds):
            ...
            for crow, vrow in zip(vectorCoords, vectorMap):
                for coords, vec in zip(crow, vrow):
                    ...
                    if influenced(dist):
                        vc = copy(vec)
                        # print(particle.v)
                        particle.applyForce(vc, dist=dist/1410)
                        # print(particle.p)
                   
            if influenced(dist):
                coords = particle.toCoords()
                ffmap[coords] = 1/(iteration+1)
...
if __name__ == '__main__':
    ...
    concArray = tdperlin(size, (1.5, 1.5, 2.5), r)
